...
- 🌐 **Network Status** — автоматическое определение режима (TUN/SOCKS5/Direct)
- 🔍 **Умная диагностика** — определение внешнего IP, проверка прокси, свежесть yt-dlp
- 🛡️ **Auto Fallback** — автоматический обход блокировок YouTube (android/tv/web клиенты)
- 🔐 **Chrome cookies** — автоматическая поддержка для приватных видео
- 📁 **Выбор папки** — сохранение в любую директорию
- ⚡ **Быстрая работа** — нативная производительность благодаря Rust
...
**yt-dlp** — единственный инструмент скачивания в проекте. Ранее использовались `lux` и `you-get`, но они удалены как устаревшие и неработающие с YouTube.

**Преимущества:**
- ✅ Поддержка 1000+ сайтов
- ✅ Обход ограничений YouTube (SABR, PO Token, Geo-blocks)
- ✅ Множество форматов и качеств
- ✅ Поддержка cookies для приватного контента
- ✅ Активная разработка и обновления (обновляется еженедельно)

**Роль в проекте:**
- Ядро скачивания видео
- Извлечение метаданных (название, длительность, превью)
- Конвертация форматов
- Обход блокировок через разные player clients (android, tv, web)
...
│  │  │  • Execute yt-dlp с fallback стратегиями    │        │   │
│  │  │  • Обход блокировок (android/tv/web)        │        │   │
│  │  │  • Monitor progress + emit events           │        │   │
│  │  └────────────────┬─────────────────────────────┘        │   │
│  │                   │                                      │   │
│  │  ┌──────────────────────────────────────────────┐        │   │
│  │  │  downloader/utils.rs (Network Detection)     │        │   │
│  │  │  • TUN mode (ifconfig utun 172.19.0.x)      │        │   │
│  │  │  • SOCKS5 (lsof sing-box LISTEN)            │        │   │
│  │  │  • System proxy (scutil --proxy)            │        │   │
│  │  │  • External IP (2ip.io, ipify)              │        │   │
│  │  └────────────────┬─────────────────────────────┘        │   │
│  └────────────────────┼──────────────────────────────────────┘   │
│                       │                                          │
│                       ▼                                          │
...
│  │                                                           │   │
│  │  ┌──────────────┐  ┌──────────────┐  ┌────────────────┐ │   │
│  │  │ yt-dlp CLI   │  │ File System  │  │ Network Tools  │ │   │
│  │  │              │  │              │  │                │ │   │
│  │  │ • Download   │  │ • Save files │  │ • ifconfig     │ │   │
│  │  │ • Extract    │  │ • Pick dir   │  │ • lsof         │ │   │
│  │  │ • Convert    │  │              │  │ • scutil       │ │   │
│  │  └──────────────┘  └──────────────┘  └────────────────┘ │   │
│  └──────────────────────────────────────────────────────────┘   │
└──────────────────────────────────────────────────────────────────┘
...
| **Network Status** | ✅ Готово | TUN/SOCKS5/Direct + IP + yt-dlp версия |
| **Auto Fallback** | ✅ Готово | android → tv → web → audio-only |
| **System Proxy** | ✅ Готово | Определение HTTP/SOCKS через scutil |
| **Версионирование** | ✅ Готово | Python скрипт + Makefile |
| **Документация** | ✅ Готово | Полный набор .md файлов |

### 🔧 Сетевая диагностика (v1.4.0+)

| Функция | Описание |
|---------|----------|
| **TUN Mode** | Определение через `ifconfig` (utun + 172.19.0.x) |
| **SOCKS5 Mode** | Определение через `pgrep sing-box` + `lsof LISTEN` |
| **System Proxy** | Определение через `scutil --proxy` (HTTP/SOCKS) |
| **External IP** | Проверка через 2ip.io, ipify.org (с anti-cache) |
| **yt-dlp Freshness** | Проверка версии и даты релиза |
...
- [x] Выбор папки
- [x] Modern UI
- [x] Network Status Bar (TUN/SOCKS5/Direct)
- [x] External IP определение
- [x] yt-dlp версия и обновление
- [x] Auto fallback (обход блокировок)
- [x] System proxy detection
